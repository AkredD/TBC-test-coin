...
interface ITransfereWithLock {
    event CreateTransferWithLock(address sender, address recipient, uint256 amount, uint lockBlockNumber);
    ...
    function transferWithLock(address recipient, uint256 amount, uint blockNumber) external returns (bool);
...
contract TBLCore is ERC20WithLiquify, ICoreStackable, ITransfereWithLock {
    ...
    constructor() ERC20WithLiquify("TBL", "TBL") {
     ...
     function transferWithLock(address recipient, uint256 amount, uint blockNumber) external override returns (bool) {
        ...
        lockedTransfers[recipient].rows.push(TransferLockRow(blockNumber, false, amount));
     ...
     function receiveLockedTransfers() external override returns (uint256) {
        ...
        for (uint i = 0; i < lockedTransfers[sender].rows.length; i++) {
            ...
            if (row.lockBlockNumber <= currentBlock && !row.received) {
                row.received = true;
